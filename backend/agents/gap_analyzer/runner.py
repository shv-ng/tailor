...
from .graph import GapAnalyzerState, compiled_graph

logger = logging.getLogger(__name__)


def run_gap_analyzer(parsed_resume: Resume, parsed_jd: Job) -> GapAnalyzerState:
    initial_state = GapAnalyzerState(parsed_resume=parsed_resume, parsed_jd=parsed_jd)
    final_state = compiled_graph.invoke(initial_state)
    if final_state.get("error"):
        logger.error(final_state.get("error"))
    return GapAnalyzerState(**final_state)
...
if __name__ == "__main__":
    ...
    print("⏳ Running Gap Analyzer Agent...")
    analysis_result = run_gap_analyzer(
        parsed_resume=parsed_resume_obj, parsed_jd=parsed_jd_obj
    )
    import pprint

    print("\n================ GAP ANALYSIS RESULTS ================")
    if analysis_result.gap_analysis:
        print(f"PROFILE SCORE: {analysis_result.gap_analysis.profile_score}/100\n")
        print("MISSING SKILLS:")
        pprint.pprint(analysis_result.gap_analysis.missing_skills)
        print("\nFULL RESPONSE OBJECT:")
        pprint.pprint(analysis_result.gap_analysis.model_dump())
    else:
